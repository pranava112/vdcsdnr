...
const useAttendanceDatamzc = (year) => {
  ...
  useEffect(() => {
    document.title = `BscMzc Year-${year} Attendance Tracker`;
    fetchData();
  }, [year]);
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = data.filter((item) => item.year === `${year}` && item.studentGroup === "bsc(mzc)");
...
export default useAttendanceDatamzc;