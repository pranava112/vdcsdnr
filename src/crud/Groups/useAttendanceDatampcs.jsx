...
const useAttendanceDatampcs = (year) => {
  ...
  useEffect(() => {
    document.title = `BscMpcs Year-${year} Attendance Tracker`;
    fetchData();
  }, [year]);
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = data.filter((item) => item.year === `${year}` && item.studentGroup === "bsc(mpcs)");
...
export default useAttendanceDatampcs;