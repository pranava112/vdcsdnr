...
const BscMpcsTable = ({ year, title, attendenceLink }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = data.filter((item) => item.year === year && item.group === "bsc(mpcs)");
                ...
                user.map((val) => (
                  <Fragment key={val.id}>
                    <tr>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{val.group}</td>
