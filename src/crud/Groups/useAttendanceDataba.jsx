...
const useAttendanceDataba = (year) => {
  ...
  useEffect(() => {
    document.title = `BA Year-${year} Attendance Tracker`;
    fetchData();
  }, [year]);
  ...
  const fetchData = async () => {
    ...
    //   axios.get("http://localhost:8080/api/vignan/student");
      ...
      const filteredData = data.filter((item) => item.year === `${year}` && item.group === "ba");
...
export default useAttendanceDataba;