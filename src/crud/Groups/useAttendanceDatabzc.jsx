...
const useAttendanceDatabzc = (year) => {
  ...
  useEffect(() => {
    document.title = `BscBzc Year-${year} Attendance Tracker`;
    fetchData();
  }, [year]);
  ...
  const fetchData = async () => {
    try {
      ...
      const filteredData = data.filter((item) => item.year === `${year}` && item.studentGroup === "bsc(bzc)");
...
export default useAttendanceDatabzc;
